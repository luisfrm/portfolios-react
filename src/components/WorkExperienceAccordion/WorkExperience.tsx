...
function ExperienceCard({ experience, isEven, index }: ExperienceCardProps) {
	...
	return (
		<AnimatedSection
			...
			delay={index * 200}
			className={cn(TIMELINE_STYLES.experienceContainer, isEven ? 'md:flex-row' : 'md:flex-row-reverse')}
		>
			...
			<Card className={CARD_STYLES.base}>
				<CardHeader className="pb-4">
					<time className={CARD_STYLES.time}>{experience.period}</time>
					<h3 className={CARD_STYLES.title}>
						{experience.position} <span className={CARD_STYLES.company}>@ {experience.company}</span>
					</h3>
				</CardHeader>
				<CardContent className={SPACING.CARD_CONTENT}>
					{experience.description && <p className={CARD_STYLES.description}>{experience.description}</p>}
					{experience.descriptionItems && (
						<ul className="list-disc list-outside pl-5 space-y-2">
							{experience.descriptionItems.map((item, index) => (
	...
	return (
		<div className={CARD_STYLES.skillsContainer}>
			{skills.map((skill, skillIndex) => (
				<Badge key={`skill-${skillIndex}`} variant="outline" className={SKILL_BADGE_STYLE}>
					{skill}
				</Badge>
			))}
		</div>
	...
	return (
		<div className={TIMELINE_STYLES.timeline}>
			...
			{experiences.map((experience: WorkExperiencesList, index: number) => (
				<ExperienceCard key={`experience-${index}`} experience={experience} isEven={index % 2 === 0} index={index} />
			))}
		</div>
	);
}
