...
const SKILLS_STYLES = {
  ...
  decorativeLine:
    "w-24 h-1 bg-gradient-to-r from-blue-600 to-slate-700 mx-auto mb-6 sm:mb-8 lg:mb-12 rounded-full",
} as const;
...
function SkillBadge({ skill }: { skill: Skill }) {
  return (
    ...
    >
      <Badge className={SKILLS_STYLES.skillBadge} variant="secondary">
...
function CategorySection({
  ...
  const categoryColorClass =
    SKILL_CATEGORY_COLORS[
      category as keyof typeof SKILL_CATEGORY_COLORS
  ...
  return (
    <AnimatedSection animation="fade-up" delay={index * 150}>
      <div className={cn(SKILLS_STYLES.categoryCard, categoryColorClass)}>
        <h3 className={SKILLS_STYLES.categoryTitle}>{category}</h3>
        <div className={SKILLS_STYLES.skillsContainer}>
          {skills.map((skill, i) => (
            <SkillBadge key={`${category}-skill-${i}`} skill={skill} />
          ))}
        </div>
      </div>
    </AnimatedSection>
    ...
    <section id="skills" className={SKILLS_STYLES.container}>
      <AnimatedSection animation="fade-scale" delay={0}>
        <div className="text-center">
          <h2 className={SKILLS_STYLES.sectionTitle}>{t("skills.title")}</h2>
          <div className={SKILLS_STYLES.decorativeLine} />
        </div>
      </AnimatedSection>
      <div className={SKILLS_STYLES.categoriesGrid}>
        {SKILLS_CATEGORIES_DATA.map((categoryData, index) => (
          <CategorySection
            key={`category-${index}`}
